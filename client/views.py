...
from tution.models import Ad, Assignee, Proposal, Question
...
def feedback(request, ad_id):
    ...
    if request.POST:
        ...
        _rating = int(request.POST.get('rating', ''))
        _feedback = request.POST.get('feedback', '')
        if _rating not in allowed_rating:
            messages.error(request, 'Invalid rating.')
            return render(request, 'client/feedback.html', context={
                'feed': get_feed(request, ad),
                'feedback': _feedback,
            })
        # print(">>>>>>>>>>>>>>>>>>>>>>>>Work", ad,not ClientFeedback.objects.filter(ad=ad).exists(),_rating,_feedback)
        if not ClientFeedback.objects.filter(ad=ad).exists():
            ClientFeedback.objects.create(ad=ad, rating=_rating, feedback=_feedback)
...
def terminate(request, ad_id):
    ...
    ad.save()
    messages.success(request, 'The tution is terminated successfully!')
    return redirect('client-applicants', ad.pk)
